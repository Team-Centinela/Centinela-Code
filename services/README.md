...
| `serverless-engine/` | Spring Boot (Java 21) on Azure Container Apps (Consumption, KEDA `azure-servicebus` scaler) | Independent scaling for burst fraud-evaluation work; rule configs change on a different cadence than case management | **Skeleton pending** — tracked in #50. Pipeline Pattern (FR-1..FR-4), `transactions.triggered_rules` writes, `FraudEvaluationCompleted` outbox |
