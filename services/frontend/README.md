...
Static SPA, served from CDN. Consult `architecture/01-overview.md` §
"External Entry Points" for the scope.

## Why static SPA

- ADR-001 chose Modular Monolith without frontend concerns in mind;
  Frontend is a thin presentation layer on top of public APIs.
- Static delivery permits easy cost minimization: $0 hosting from Azure
  Static Web Apps free tier (no APIM, no premium tier) — fits the
  $60 budget window.
