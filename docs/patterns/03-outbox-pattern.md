...
4. Consumer-side idempotency (mandated by `ADR-003` §3.3 and detailed in `patterns/06-idempotency-key.md`) makes the post-restart burst a non-event from the consumers' perspective; the same `transactionId` arriving twice does not produce two scores or two cases.
