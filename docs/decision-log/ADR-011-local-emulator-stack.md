...
**Azure SQL Edge retirement** (Microsoft, 2025-09-30). The Service Bus Emulator's state store historically depended on `mcr.microsoft.com/azure-sql-edge:latest`; that image is officially retired, and MCR's continued service is courtesy of Microsoft's backward-compatibility policy, not a guarantee. Microsoft's own [official Service Bus Emulator compose template](https://github.com/Azure/azure-service-bus-emulator-installer) migrated to `mcr.microsoft.com/mssql/server:2022-latest` in 2025; `docker-compose.yml` follows that migration. **Inherited upstream risk** — if Microsoft further retires SQL Server Linux 2022 in favour of 2025, the SB Emulator may follow on a different timeline than Centinela. Watch [`Azure/azure-service-bus-emulator-installer`](https://github.com/Azure/azure-service-bus-emulator-installer) releases for upstream coordination; the digest pin in §11.2 makes the dependency visible and auditable.
...
- **ADR-010 — issue/PR discipline** — Markdown lives on branch `feat/adr-010-issue-pr-discipline-implementation` as of 2026-07-28; **NOT merged to `feat/week-one-consolidation`**. Phase 0 should open an issue to land ADR-010 before Phase 2 apply (every `[E.A*]` companion close-out cites ADR-010 §10.4; without the markdown the close-out template is informal). This ADR's §11.6 layer 2 depends on ADR-010 being on disk.
...
- Microsoft Service Bus Emulator — [`Azure/azure-service-bus-emulator-installer`](https://github.com/Azure/azure-service-bus-emulator-installer) (compose template + SB Emulator sample)
...
- Microsoft Docs — [Test locally by using the Azure Service Bus emulator](https://github.com/MicrosoftDocs/azure-docs/blob/main/articles/service-bus-messaging/test-locally-with-service-bus-emulator.md) (last reviewed 2025-10-27; uses `mssql/server:2022-latest` as the SQL backend)
...
- SQL Edge EOL + SB Emulator migration discussion — [`Azure/azure-service-bus-emulator-installer#18`](https://github.com/Azure/azure-service-bus-emulator-installer/issues/18) (Microsoft's own installer repo recommending `mssql/server` instead of `azure-sql-edge`)
