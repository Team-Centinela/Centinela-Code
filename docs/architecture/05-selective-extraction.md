...
| **Runtime** | Azure Container Apps (Consumption plan) with **KEDA `azure-servicebus` scaler** bound to the `transactions-raw` queue — scales out per active-message count, scales to **zero replicas** when the queue is empty. Per-second billing. First 180,000 vCPU-seconds and 2 M requests / month / subscription are free. |
